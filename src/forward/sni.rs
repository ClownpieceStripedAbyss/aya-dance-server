/// Adapted from https://github.com/branlwyd/rspd/blob/master/src/main.rs
use std::{
  cmp::min,
  io::ErrorKind,
  mem,
        ...
        HandshakeRecordReaderReading::ContentType => {
          ...
          match this.reader.as_mut().poll_read(cx, &mut buf) {
            Poll::Ready(Ok(())) if buf.filled().len() == 1 => {
              ...
              *this.currently_reading = HandshakeRecordReaderReading::MajorMinorVersion(0);
            }
            rslt => return rslt,
          }
        }
          ...
          match this.reader.as_mut().poll_read(cx, &mut buf) {
            Poll::Ready(Ok(())) => {
              *bytes_read = buf.filled().len();
              if *bytes_read == 2 {
                *this.currently_reading = HandshakeRecordReaderReading::RecordSize([0, 0], 0);
              }
            }
            rslt => return rslt,
          }
              ...
              *bytes_read = buf.filled().len();
              if *bytes_read == 2 {
                ...
                if record_size > MAX_RECORD_SIZE {
                  return Poll::Ready(Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("record too large ({} > {})", record_size, MAX_RECORD_SIZE),
                  )));
                }
                *this.currently_reading = HandshakeRecordReaderReading::Record(record_size)
              }
            }
            rslt => return rslt,
          }
...
async fn read_sni_host_name_from_client_hello<R: AsyncRead>(
  ...
  // Session ID (u8-length vec), cipher suites (u16-length vec), compression
  // methods (u8-length vec).
